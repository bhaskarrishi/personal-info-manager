...
class PasswordVaultWindow(QWidget):
    ...
    def init_ui(self):
        ...
        self.toggle_sensitive_btn = QPushButton('Show Passwords')
    ...
    def toggle_sensitive_data(self):
        """Toggle visibility of passwords in table."""
        self.show_sensitive = not self.show_sensitive
        if self.show_sensitive:
            self.toggle_sensitive_btn.setText('Hide Passwords')
        else:
            self.toggle_sensitive_btn.setText('Show Passwords')
        self.load_passwords()
        ...
        try:
            ...
            if results:
                for row_data in results:
                    ...
                    # Show password if toggled, otherwise show dots
                    if self.show_sensitive:
                        try:
                            decrypted = self.encryption.decrypt(row_data['encrypted_password'])
                            self.table.setItem(row_position, 2, QTableWidgetItem(decrypted))
                        except:
    ...
    def delete_password(self):
        ...
        dialog = ConfirmationDialog(
            'Confirm Delete',
            f'Are you sure you want to permanently delete the password for "{service_name}"?\\n\\nThis action cannot be undone.',
            self
        )
